...
唯一的领域校准由上层 CUMCM 发布门执行，不属于模型自行降级：`audit_academic_candidate.py`
在 `SCENE=MODELING` 且进入国赛长稿链时，实际运行
`audit_lexical_corpus_calibration.py`，用 59 篇已核验全文逐词复计。一个短语只有在至少 5 篇
不同论文中出现、且当前位置没有合并命中普通结构信号时，才从“单次硬阻断”降为
`REVIEW_CONTEXT`；原位置、论文数和段落数仍写入报告，重复节奏和空泛功能继续由其他门检查。
该校准不能用于一般中文、研究论文、课程笔记或单独调用本 Skill，也不构成作者身份判断。
...
上层 `$aigc-writing-router` 的统一学术门可用 `--humanize-run <run-dir>` 重放本 run，并复用
其中源稿继承的 strict 技术短语 KEEP。复用条件包括 `run/v3`、before/after 哈希、当前 scanner
和 lexicon 哈希、精确位置、finding hash、具体功能理由以及 `emit` 重放；它不把 KEEP 升格为
真人审批，也不清除任何语义、数学或保护项 warning。不要另造 decisions 文件重复声明同一 KEEP。
...
“单个 strict 命中必须记录”不等于“任何体裁中单个词都必须删除”。CUMCM 上层校准仍保留
finding，只纠正其证据强度：跨至少 5 篇获奖论文的人类常用短语不能单凭一次出现成为 AI 专属
硬证据；与普通结构病灶合并命中时不豁免。其他场景仍执行本 Skill 的原始严格处置。
...
| 来源动作候选审计 | `corpus-action-sources.json` + `build_humanize_action_profile.py` + `validate_humanize_candidate_queue.py` |
| 来源分类 | `source-provenance-trust.json` |
| 可复用 Prompt | `system-prompt-contract.md` |
| 验证 Skill | `evaluation-contract.md` |
| 生成资格审计 | `generator-projection-policy.json` + `build_humanize_generator_projection.py` + qualification scripts |
...
生成资格属于开发审计，不属于普通改写。完整 manifest、projection、runner、隔离证据和三态裁决只按 [evaluation-contract.md](references/evaluation-contract.md) 第 16.2 节执行。本地 runner 最高只能形成 E2 capture；没有外部签名隔离证据和完整原子矩阵时，资格固定为 `NOT_EVALUATED`。不得用 validator PASS、单元测试、规则数量或零散盲评升级生成模型资格。

安装版审计可使用 replay/capture 工具核验记录和父进程观察；其 PASS 只覆盖对应 self-consistency 范围，不向生成资格贡献 PASS。

## 来源锚定的改写候选

普通 `REWRITE/DRAFT` 固定按原稿、supplied content、场景规则和已验证 Voice Profile 工作；当前没有 production positive action card。GPT 生成或归属未解决的 MD/TeX 只能形成带 detector 的负例，不能成为真人范文、PERSONAL Voice、事实来源或可复制句库。乱码、失效行范围和被排除文件不得继续提供卡片。

开发者审计来源动作时才读取 `workflow.md` 第 3.1.1 节、`source-provenance-trust.json` 和 `corpus-action-sources.json`，并使用 `build_humanize_action_profile.py` / `validate_humanize_candidate_queue.py`。候选队列、warning proposal 和来源复制检查都是安装版审计面，不是普通提示词；本地 proposal 固定为未验证且保持 `REVIEW/2`。
...
<!-- GENERATOR_PROJECTION_CONTROL_BEGIN:SECOND_PASS -->
fresh second pass 只按 [long-document-workflow.md](references/long-document-workflow.md) 第 16 节运行，并绑定当前 chunk、Voice、projection 和真实 trial。缺 trial 或 fresh `REWRITE` 为 REVIEW；证据漂移为 FAIL；未运行真实 runner 固定为 `NOT_RUN`。
<!-- GENERATOR_PROJECTION_CONTROL_END:SECOND_PASS -->
