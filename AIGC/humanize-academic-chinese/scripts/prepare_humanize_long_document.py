...
SOURCE_ROLES = {"AUTHOR_TEXT", "GENERATED", "UNRESOLVED"}
SOURCE_ROLE_OVERRIDE_ROLES = {"AUTHOR_TEXT", "GENERATED"}
SOURCE_ROLE_OVERRIDE_SCHEMA = "humanize-source-role-overrides/v1"
SOURCE_ROLE_OVERRIDE_RESOLUTION_SCHEMA = (
    "humanize-source-role-override-resolution/v1"
)
SOURCE_ROLE_OVERRIDE_AUTHORITY = (
    "CALLER_SCOPE_OVERRIDE_ONLY_NOT_AUTHORSHIP_OR_QUALITY_ATTESTATION"
)
GENERATED_PATH_AUXILIARY_SEGMENTS = {"generated", "build", "autogen", "dist"}
GENERATED_MARKER_PATTERNS = (
    (
        "TEX_COMMENT_EN_GENERATED_BY",
        re.compile(
            r"(?:this\s+file\s+(?:is|was)\s+)?"
            r"(?:(?:auto(?:matically)?[- ]?)?generated)\s+by\s+\S.{0,240}",
            re.IGNORECASE,
        ),
    ),
    (
        "TEX_COMMENT_EN_AUTOGENERATED_DO_NOT_EDIT",
        re.compile(
            r"(?:(?:this\s+is\s+an?\s+)?(?:auto[- ]?generated|automatically\s+generated)"
            r"(?:\s+file)?|generated\s+file)"
            r".{0,160}(?:do\s+not\s+(?:edit|modify)|changes?\s+will\s+be\s+overwritten)"
            r"[.! ]*",
            re.IGNORECASE,
        ),
    ),
    (
        "TEX_COMMENT_EN_DO_NOT_EDIT_GENERATED",
        re.compile(
            r"do\s+not\s+(?:edit|modify).{0,160}"
            r"(?:auto[- ]?generated|automatically\s+generated|generated\s+file)[.! ]*",
            re.IGNORECASE,
        ),
    ),
    (
        ...
        re.compile(
            r"[本此]文件由.{1,120}自动生成(?:[，,；;。.]*(?:请勿(?:手工)?(?:编辑|修改)))?[。.! ]*"
        ),
    ),
    (
        "TEX_COMMENT_ZH_AUTOGENERATED_DO_NOT_EDIT",
        re.compile(
            r"(?:自动生成(?:的)?文件|[本此]文件自动生成)"
            r"[，,；;。.]*(?:请勿(?:手工)?(?:编辑|修改)|修改将被覆盖)[。.! ]*"
        ),
    ),
)
...
def _prepare_artifact_paths(output: Path) -> list[Path]:
    paths = [
        ...
        output / "source_role_overrides.json",
...
def _tex_structure_view(text: str) -> str:
    """Mask TeX code and comments without changing source offsets or line endings.

    Every downstream structure reader must use this view rather than inventing a
    second comment/verb parser.  Keeping ``\r`` and ``\n`` intact lets regex
    offsets, line inventories and protected-span ledgers remain anchored to the
    original source bytes.
    """
    return lexical._tex_structure_view(text)


def discover_tex_includes(path: Path, text: str) -> list[tuple[str, Path]]:
    discovered: list[tuple[str, Path]] = []
    structure = _tex_structure_view(text)
    for match in INCLUDE_RE.finditer(structure):
        raw_target = text[match.start("path") : match.end("path")].strip()
...
def _reject_duplicate_json_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise ValueError("duplicate source-role override JSON key")
        result[key] = value
    return result


def load_source_role_overrides(spec_path: Path | None) -> dict[str, Any]:
    """Load a strict, path-resolved caller scope override request.

    The resulting artifact is always frozen into the prepare closure.  It can
    alter authoring scope, but it is deliberately not an authorship or quality
    attestation.
    """
    if spec_path is None:
        return {
            "schema_version": SOURCE_ROLE_OVERRIDE_RESOLUTION_SCHEMA,
            "authority": SOURCE_ROLE_OVERRIDE_AUTHORITY,
            "request_supplied": False,
            "request_sha256": "",
            "overrides": [],
        }
    resolved_spec = spec_path.resolve()
    raw = resolved_spec.read_bytes()
    try:
        payload = json.loads(
            raw.decode("utf-8"), object_pairs_hook=_reject_duplicate_json_keys
        )
    except UnicodeDecodeError as error:
        raise ValueError("source-role override must be UTF-8") from error
    if not isinstance(payload, dict) or set(payload) != {"schema_version", "overrides"}:
        raise ValueError("source-role override fields mismatch")
    if payload.get("schema_version") != SOURCE_ROLE_OVERRIDE_SCHEMA:
        raise ValueError("source-role override schema mismatch")
    entries = payload.get("overrides")
    if not isinstance(entries, list):
        raise ValueError("source-role overrides must be a list")
    normalized: list[dict[str, Any]] = []
    seen_paths: set[str] = set()
    for entry in entries:
        if not isinstance(entry, dict) or set(entry) != {"path", "source_role", "reason"}:
            raise ValueError("source-role override entry fields mismatch")
        requested_path = entry.get("path")
        role = entry.get("source_role")
        reason = entry.get("reason")
        if not isinstance(requested_path, str) or not requested_path.strip():
            raise ValueError("source-role override path is invalid")
        if role not in SOURCE_ROLE_OVERRIDE_ROLES:
            raise ValueError("source-role override role is invalid")
        if (
            ...
            raise ValueError("source-role override reason is invalid")
        target = Path(requested_path)
        if not target.is_absolute():
            target = resolved_spec.parent / target
        target = target.resolve()
        key = _path_key(target)
        if key in seen_paths:
            raise ValueError("duplicate source-role override target")
        seen_paths.add(key)
        identity = {
        ...
        normalized.append(
            {
                "override_id": "SRO-" + canonical_sha256(identity)[:16],
                **identity,
                "applied_files": [],
            }
        )
    normalized.sort(key=lambda item: str(item["resolved_path"]).casefold())
    return {
        "schema_version": SOURCE_ROLE_OVERRIDE_RESOLUTION_SCHEMA,
        "authority": SOURCE_ROLE_OVERRIDE_AUTHORITY,
...
def validate_source_role_override_artifact(payload: Any) -> dict[str, dict[str, Any]]:
    if not isinstance(payload, dict) or set(payload) != {
        ...
        raise ValueError("source-role override resolution fields mismatch")
    if payload.get("schema_version") != SOURCE_ROLE_OVERRIDE_RESOLUTION_SCHEMA:
        raise ValueError("source-role override resolution schema mismatch")
    if payload.get("authority") != SOURCE_ROLE_OVERRIDE_AUTHORITY:
        raise ValueError("source-role override authority mismatch")
    supplied = payload.get("request_supplied")
    request_sha = payload.get("request_sha256")
    if not isinstance(supplied, bool):
        raise ValueError("source-role override request flag is invalid")
    if supplied:
        if not isinstance(request_sha, str) or not re.fullmatch(r"[0-9a-f]{64}", request_sha):
            raise ValueError("source-role override request hash is invalid")
    elif request_sha != "":
        raise ValueError("absent source-role override has a request hash")
    entries = payload.get("overrides")
    if not isinstance(entries, list) or (not supplied and entries):
        raise ValueError("source-role override resolution entries are invalid")
    ...
    for entry in entries:
        if not isinstance(entry, dict) or set(entry) != {
            ...
            raise ValueError("source-role override resolution entry fields mismatch")
        override_id = entry.get("override_id")
        resolved_path = entry.get("resolved_path")
        requested_path = entry.get("requested_path")
        role = entry.get("source_role")
        reason = entry.get("reason")
        applied = entry.get("applied_files")
        if not isinstance(override_id, str) or not re.fullmatch(r"SRO-[0-9a-f]{16}", override_id):
            raise ValueError("source-role override id is invalid")
        if override_id in seen_ids:
            raise ValueError("duplicate source-role override id")
        seen_ids.add(override_id)
        if not isinstance(resolved_path, str) or not Path(resolved_path).is_absolute():
            raise ValueError("source-role override resolved path is invalid")
        if not isinstance(requested_path, str) or not requested_path.strip():
            raise ValueError("source-role override requested path is invalid")
        if role not in SOURCE_ROLE_OVERRIDE_ROLES:
            raise ValueError("source-role override role is invalid")
        if not isinstance(reason, str) or not reason or len(reason) > 500:
            raise ValueError("source-role override reason is invalid")
        if not isinstance(applied, list) or len(applied) > 1:
            raise ValueError("source-role override applied files are invalid")
        for applied_file in applied:
            if not isinstance(applied_file, dict) or set(applied_file) != {
                "file_id",
                "snapshot_sha256",
            }:
                raise ValueError("source-role override applied file fields mismatch")
            if not isinstance(applied_file.get("file_id"), str) or not re.fullmatch(
                r"F[0-9]{5}", str(applied_file.get("file_id"))
            ):
                raise ValueError("source-role override applied file id is invalid")
            if not isinstance(applied_file.get("snapshot_sha256"), str) or not re.fullmatch(
                r"[0-9a-f]{64}", str(applied_file.get("snapshot_sha256"))
            ):
                raise ValueError("source-role override applied file hash is invalid")
        identity = {
            "requested_path": requested_path,
        ...
        if override_id != "SRO-" + canonical_sha256(identity)[:16]:
            raise ValueError("source-role override id binding mismatch")
        key = _path_key(Path(resolved_path))
        if key in by_path or (previous_key and key < previous_key):
            raise ValueError("source-role override paths are duplicate or unsorted")
        previous_key = key
        by_path[key] = entry
    return by_path


def deterministic_tex_generation_markers(text: str) -> tuple[list[dict[str, Any]], list[str]]:
    """Recognize only finite registry markers in executable TeX comments."""
    code_spans, problems = lexical._tex_code_like_spans(text)
    code_ranges = [(start, end) for start, end, _reason in code_spans]
    comments = lexical._tex_comment_ranges(text, code_ranges)
    evidence: list[dict[str, Any]] = []
    starts = _line_starts(text)
    for start, end in comments:
        payload = text[start + 1 : end].strip()
        for marker_id, pattern in GENERATED_MARKER_PATTERNS:
            if pattern.fullmatch(payload):
                evidence.append(
                    {
                        "kind": "DETERMINISTIC_TEX_COMMENT_MARKER",
                        "marker_id": marker_id,
                        "line": _offset_to_line(starts, start),
                        "comment_sha256": sha256(payload.encode("utf-8")),
                    }
                )
                break
    return evidence, list(dict.fromkeys(problems))
...
def classify_source_role(
    ...
    """Return source role, bounded evidence, and authoring processing state."""
    evidence: list[dict[str, Any]] = [
        {"kind": "DISCOVERY_RELATION", "relation": str(record.get("relation", ""))}
    ]
    status = str(record.get("status", ""))
    if text is None or status in {"UNRESOLVED", "UNRESOLVED_INCLUDE", "SKIPPED_GARBLED"}:
        evidence.append({"kind": "SOURCE_UNAVAILABLE", "status": status})
        return "UNRESOLVED", evidence, "UNRESOLVED_SOURCE_ROLE_NO_AUTHORING"

    path_segments = sorted(
        {
            part.casefold()
            for part in Path(str(record["path"])).resolve().parts
            if part.casefold() in GENERATED_PATH_AUXILIARY_SEGMENTS
        }
    )
    if path_segments:
        evidence.append(
            {"kind": "AUXILIARY_PATH_SEGMENT", "segments": path_segments}
        )

    markers: list[dict[str, Any]] = []
    parse_problems: list[str] = []
    if str(record.get("suffix", "")).lower() in {".tex", ".ltx"}:
        markers, parse_problems = deterministic_tex_generation_markers(text)
        evidence.extend(markers)
        if parse_problems:
            evidence.append(
                {
                    "kind": "TEX_MARKER_SCAN_INCOMPLETE",
                    "problem_codes": sorted(parse_problems)[:8],
                    "problem_count": len(parse_problems),
                }
            )
    ...
    if override is not None:
        evidence.append(
            {
                "kind": "CALLER_SCOPE_OVERRIDE",
                "override_id": override["override_id"],
                "source_role": override["source_role"],
                "authority": SOURCE_ROLE_OVERRIDE_AUTHORITY,
            }
        )
        role = str(override["source_role"])
    elif markers and parse_problems:
        role = "UNRESOLVED"
    elif markers:
        role = "GENERATED"
    elif path_segments:
        ...
        evidence.append(
            {
                "kind": "INSUFFICIENT_GENERATED_EVIDENCE",
                "reason": "PATH_SEGMENT_IS_AUXILIARY_ONLY",
            }
        )
    else:
        role = "AUTHOR_TEXT"
        evidence.append(
            {
                "kind": "DEFAULT_AUTHOR_SCOPE",
                "reason": "READABLE_SEED_OR_EXPLICIT_TEX_INCLUDE_WITHOUT_GENERATED_SIGNAL",
            }
        )

    if status == "CHANGED_AFTER_SNAPSHOT":
        processing = "CHANGED_AFTER_SNAPSHOT_NO_AUTHORING"
    elif role == "AUTHOR_TEXT":
        processing = (
            "AUTHORING_QUEUE_BY_CALLER_OVERRIDE"
            if override is not None
            else "AUTHORING_QUEUE"
        )
    elif role == "GENERATED":
        processing = (
            "RETAINED_GENERATED_NO_AUTHORING_BY_CALLER_OVERRIDE"
            if override is not None
            else "RETAINED_GENERATED_NO_AUTHORING"
        )
    else:
        processing = "UNRESOLVED_SOURCE_ROLE_NO_AUTHORING"
    return role, evidence, processing


def assign_source_roles(
    files: Sequence[dict[str, Any]],
    texts: dict[str, str],
    override_artifact: dict[str, Any],
) -> None:
    overrides = validate_source_role_override_artifact(override_artifact)
    applied_files: dict[str, list[dict[str, str]]] = {key: [] for key in overrides}
    for record in files:
        override = None
        if record.get("snapshot_copy"):
            key = _path_key(Path(str(record["path"])))
            ...
            if override is not None:
                if str(record.get("status", "")) in {
                    ...
                    raise ValueError("source-role override target is not readable")
                snapshot_sha256 = str(record.get("sha256", ""))
                if not re.fullmatch(r"[0-9a-f]{64}", snapshot_sha256):
                    raise ValueError("source-role override target snapshot hash is invalid")
                applied_files[key].append(
                    {
                        "file_id": str(record["file_id"]),
                        "snapshot_sha256": snapshot_sha256,
                    }
                )
        role, evidence, processing = classify_source_role(
            record, texts.get(str(record["file_id"])), override
        )
        record["source_role"] = role
        record["source_role_evidence"] = json.dumps(
            evidence, ensure_ascii=False, sort_keys=True, separators=(",", ":")
        )
        record["source_processing_status"] = processing
    for key, override in overrides.items():
        matched = applied_files[key]
        if len(matched) != 1:
            raise ValueError("source-role override target must match exactly one readable file")
        declared = list(override.get("applied_files", []))
        if declared and declared != matched:
            raise ValueError("source-role override applied-file binding mismatch")
        override["applied_files"] = matched
...
def _tex_group_end(text: str, start: int, opening: str, closing: str) -> int | None:
    return lexical._tex_group_end(text, start, opening, closing)
...
def _tex_command_call_spans(
    ...
    *,
    ...
    return lexical._tex_command_call_spans(
        text,
...
def _trim_authenticated_heading_spans(
    spans: Sequence[tuple[int, int, str]],
    heading_analysis: dict[str, list[Any]],
) -> list[tuple[int, int, str]]:
    heading_end_by_start = {
        int(heading["start"]): int(heading["end"])
        for heading in heading_analysis["headings"]
    }
    return [
        (start, heading_end_by_start.get(start, end), reason)
...
def _extra_protected_spans(
    ...
    if suffix in {".tex", ".ltx"}:
        structure = _tex_structure_view(text)
        heading_analysis = repetition_guards.analyze_tex_headings(text)
        spans.extend(
            (
                int(heading["line_start"]),
                int(heading["end"]),
                "locked-heading",
            )
            for heading in heading_analysis["headings"]
        ...
        spans.extend(
            (
                int(span["start"]),
                int(span["end"]),
                "malformed-locked-heading",
            )
            for span in heading_analysis["malformed_spans"]
        )
        command_spans = _tex_command_call_spans(
            text, editable_style_wrappers, structure_view=structure
        )
        spans.extend(
            _trim_authenticated_heading_spans(command_spans, heading_analysis)
        )
    else:
        spans.extend(lexical._markdown_structure_spans(text))
    return spans
...
def protected_spans(
    ...
    if document_format == "tex":
        raw = _trim_authenticated_heading_spans(
            raw,
            repetition_guards.analyze_tex_headings(text),
        )
...
def _environment_problems(text: str, suffix: str) -> list[str]:
    ...
    stack: list[str] = []
    problems: list[str] = []
    for match in TEX_ENV_RE.finditer(_tex_structure_view(text)):
    ...
    problems.extend(repetition_guards.analyze_tex_headings(text)["problems"])
    return problems[:20]


def _heading_regions(text: str, suffix: str) -> list[dict[str, Any]]:
    tex_headings = (
        repetition_guards.analyze_tex_headings(text)["headings"]
        if suffix in {".tex", ".ltx"}
        else []
    )
    markdown_matches = (
        [] if suffix in {".tex", ".ltx"} else list(MD_HEADING_RE.finditer(text))
    )
    document_body_start: int | None = None
    if suffix in {".tex", ".ltx"}:
        for match in TEX_ENV_RE.finditer(_tex_structure_view(text)):
            if match.group("kind") == "begin" and match.group("name") == "document":
                document_body_start = match.end()
                break
    heading_starts = [
        *(int(heading["line_start"]) for heading in tex_headings),
        *(match.start() for match in markdown_matches),
    ]
    body_boundaries = [document_body_start] if document_body_start is not None else []
    boundaries = [0, *body_boundaries, *heading_starts, len(text)]
    boundaries = sorted(set(boundaries))
    tex_heading_by_start = {
        int(heading["line_start"]): heading for heading in tex_headings
    }
    markdown_heading_by_start = {
        match.start(): match for match in markdown_matches
    }
    hierarchy: list[str] = []
    level_map = {
        "part": 1,
    ...
    for start, end in zip(boundaries, boundaries[1:]):
        tex_heading = tex_heading_by_start.get(start)
        markdown_heading = markdown_heading_by_start.get(start)
        if tex_heading is not None or markdown_heading is not None:
            if tex_heading is not None:
                level = level_map[str(tex_heading["level"])]
                title = str(tex_heading["heading_leaf"])
            else:
                assert markdown_heading is not None
                level = len(markdown_heading.group("marks"))
                title = markdown_heading.group("title")
        ...
        if end > start:
            regions.append({
                ...
                "region_kind": (
                    "tex-preamble"
                    if document_body_start is not None and end <= document_body_start
                    else "content"
                ),
            })
    return regions
...
def _safe_split_offsets(text: str, start: int, end: int, spans: Sequence[dict[str, Any]], suffix: str) -> list[int]:
    ...
    structure_segment = (
        _tex_structure_view(text)[start:end]
        if suffix in {".tex", ".ltx"}
        else segment
    )
    offsets: list[int] = []
    ...
    for line, clean in zip(
        segment.splitlines(keepends=True), structure_segment.splitlines(keepends=True)
    ):
        if suffix in {".tex", ".ltx"}:
...
def build_units(
    ...
    *,
    ...
    for region in _heading_regions(text, suffix):
        locked_preamble = region.get("region_kind") == "tex-preamble"
        parts = _split_region(
            text,
            region,
            spans,
        ...
        for part_index, (start, end, over_limit) in enumerate(parts, 1):
            ...
            problems = [] if locked_preamble else list(
                dict.fromkeys(
                    [
                        *_environment_problems(raw, suffix),
                        *lexical._tex_protection_problems(
                            raw,
                            "tex" if suffix in {".tex", ".ltx"} else "markdown",
                        ),
                    ]
                )
            )
            ...
            if locked_preamble:
                ...
                status = "SKIPPED_PROTECTED"
            elif crossing or over_limit or route["status"] == "AMBIGUOUS":
                status = "UNRESOLVED"
            else:
                status = "PENDING"
            notes: list[str] = []
            if locked_preamble:
                notes.append("tex_preamble_locked")
            if crossing:
...
def prepare(
    ...
    *,
    ...
    voice_sample_spec: Path | None = None,
    voice_allowed_root: Path | None = None,
    source_role_overrides: Path | None = None,
) -> dict[str, Any]:
    requested_scene = scene.upper()
    ...
    source_role_override_artifact = load_source_role_overrides(source_role_overrides)
    ...
    assign_source_roles(files, texts, source_role_override_artifact)
    _write_json(output / "source_role_overrides.json", source_role_override_artifact)
    ...
    source_roles = Counter(record["source_role"] for record in files)
    source_processing_statuses = Counter(
        record["source_processing_status"] for record in files
    )
    ...
    ) or source_roles.get("UNRESOLVED", 0) > 0 or processable_editable_units == 0 else "READY"
    metadata = {
        ...
        "source_role_override_status": (
            "APPLIED"
            if source_role_override_artifact["overrides"]
            else "NOT_SUPPLIED"
        ),
        "source_role_overrides": "source_role_overrides.json",
        "source_role_overrides_sha256": sha256(
            (output / "source_role_overrides.json").read_bytes()
        ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "strict humanize-source-role-overrides/v1 JSON; caller scope override "
            "is frozen and never treated as authorship or quality attestation"
        ),
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        metadata = prepare(
            ...
            source_role_overrides=args.source_role_overrides,
        )
