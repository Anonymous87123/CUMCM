...
export function loadConfig(path = resolve(REPO_ROOT, '.patina.default.yaml'), { overridePath } = {}) {
  ...
  // Respect an explicit HOME on Windows as well as POSIX. node:os.homedir()
  // follows USERPROFILE on Windows and otherwise makes test/portable HOME
  ...
  for (const userPath of [...new Set([resolve(configuredHome, '.patina.yaml'), resolve(process.cwd(), '.patina.yaml')])]) {
