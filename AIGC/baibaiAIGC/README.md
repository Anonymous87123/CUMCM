...
一个用于中文论文、摘要、课程作业和技术文档生成保守改写候选的工作目录。

> Codex Skill 当前策略：第一轮是默认候选；第二轮不再自动执行，只在用户
> 明确要求或审阅第一轮后明确批准时做克制的终稿校正。数学建模、工程、
> 证明、法律和医学文本默认不运行第二轮。调用名为 `$baibai-aigc`。
...
- 每次调用最多处理一轮；第一轮完成后可以直接停止。
- 第二轮是显式选择，不是第一轮之后的必经步骤。
- 单轮内部也必须先分块，再逐块改写，最后按原段落结构还原。
- 不新增事实、数据、文献、案例或结论。
- 保留原文术语、编号、逻辑关系和论文语体。
- 多工具场景从同一冻结原文生成候选，不把一个改写器的输出自动交给另一个改写器继续重写。
...
- 需要补写建模依据、实验过程或学术证据的任务；这些内容应先交给相应领域 Skill
