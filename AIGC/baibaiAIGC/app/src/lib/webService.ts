...
export const webService: AppService = {
  ...
  async startRunRound(sourcePath: string, modelConfig: ModelConfig, roundNumber: number): Promise<string | null> {
    const { runId } = await requestJson<{ runId: string }>("/api/run-round", {
      method: "POST",
      body: JSON.stringify({ sourcePath, modelConfig, roundNumber }),
    });
    return runId;
  },
