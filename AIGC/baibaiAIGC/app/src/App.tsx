...
export function App({ service, pickerLabel }: Props) {
  ...
  async function handleRunRound() {
    ...
    try {
      ...
      setProgress(null);
      progressUnlistenRef.current?.();
      const approvedRound = documentStatus.nextRound;
      const runToken = await service.startRunRound(documentStatus.sourcePath, modelConfig, approvedRound);
      ...
      const result = await service.awaitRunRound(documentStatus.sourcePath, modelConfig, runToken, approvedRound);
      progressUnlistenRef.current?.();
      progressUnlistenRef.current = null;
      setProgress(null);
