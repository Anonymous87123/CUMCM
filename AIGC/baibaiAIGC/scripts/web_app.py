...
def run_round_async(
    ...
    try:
        ...
        try:
            result = run_round_for_app(source_path, model_config, round_number)
        finally:
...
def post_run_round() -> tuple[Response, int] | Response:
    try:
        ...
        if round_number not in {1, 2}:
            raise ValueError("roundNumber must be 1 or 2.")
        run_id = uuid.uuid4().hex
        RUN_STATES[run_id] = ProgressState()
        worker = threading.Thread(
            target=run_round_async,
            args=(run_id, source_path, model_config, round_number),
            daemon=True,
        )
        worker.start()
