...
def detect_next_round(doc_id: str, prompt_profile: str = "cn") -> int:
    ...
    if state.prompt_profile == "cn" and state.next_round > 1:
        raise ValueError(
            "Round 2 is optional and requires explicit approval; "
            "call build_round_context(..., round_number=2) or "
            "run_skill_round(..., round_number=2)."
        )
