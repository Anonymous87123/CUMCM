...
This is a general prose editor, not the content authority for academic,
mathematical-modeling, legal, or other evidence-constrained documents. When the
input is CUMCM or Chinese academic prose, route through `$aigc-writing-router`
and use the applicable domain skill before any style edit. Do not apply this
skill after another editor as an automatic second rewrite.

For files containing TeX, math, code, structured data, result literals, labels,
or citation keys, treat those spans as byte-for-byte protected. Edit only the
surrounding prose when the caller has supplied an explicit protected-span map;
otherwise diagnose the prose and return the proposed local edits without
rewriting the file.
...
**File mode.** The user points at a file. Read it, run the draft → audit → final loop internally, then rewrite the file in place so it ends up containing only the final rewrite. Humanize the prose only: leave code blocks, frontmatter, data, link targets, TeX commands and environments, math, labels, citation keys, numbers, units, and result literals untouched. For mixed prose/TeX files, require a protected-span map before writing in place; without one, return diagnostics and local proposals only. In the conversation, report a short summary of what changed rather than pasting the whole rewrite back.
