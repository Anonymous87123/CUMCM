...
% 提交模式必须明确选择 AI 声明。启用 AI 时还必须给出非空用途说明，
% 防止把模板占位语带入最终 PDF。
\ifdefined\MCMSubmission
  \ifdefined\MCMUsesAI
    \ifdefined\MCMNoAI
      \PackageError{cumcm-template}
        {MCMUsesAI and MCMNoAI cannot be enabled together}
        {Keep exactly one AI declaration switch in submission.tex.}
    \fi
    \ifdefined\MCMUseDescription
      \ifx\MCMUseDescription\empty
        \PackageError{cumcm-template}
          {MCMUseDescription cannot be empty}
          {Describe the actual AI use before compiling the submission version.}
      \fi
    \else
      \PackageError{cumcm-template}
        {MCMUsesAI requires MCMUseDescription}
        {Define a truthful non-empty AI use description in submission.tex.}
    \fi
  \else
    \ifdefined\MCMNoAI\else
      \PackageError{cumcm-template}
        {Choose MCMUsesAI or MCMNoAI before submission}
        {Define exactly one declaration switch in submission.tex.}
    \fi
  \fi
\fi
...
\ifmcmguidance
  ...
  \begin{mcmguidebox}
    ...
    \MCMGuideField{段落推进}{不设统一句序。根据本段实际出现的证据，从数据读数、方程性质、物理后果、试算结果、计算负担或前问接口中选择自然起点，只留下读者理解当前选择所需的判断、动作和边界；没有真实取舍时不补造候选。每个“故、因此、考虑到、由图可知”之前有可定位依据，之后有变量、公式、算法或决策动作。}
  ...
  \begin{table}[H]
    \centering
    \small
    \caption{段落判断备忘（仅在存在真实取舍时按需填写，不决定正文句序）}
...
\begin{abstract}
\MCMGuide
  {在一页内交代背景任务、逐问处理和主要结论，使摘要脱离正文仍能说明“研究什么、怎样算、得到什么”。}
  {总任务；逐问模型链；最有判别力的数值或关系；约束满足情况；至少一项检验；4--6 个关键词。}
  {按“虎头—猪肚—豹尾”压缩成三段：第一段交代背景、对象和全部任务；第二段按问题写处理思路、数据动作、模型/算法和关键结果；第三段只写有证据的创新、稳定性或应用价值。每段从本题对象起笔，不把算法名堆成清单。}
  {\MCMCorpusScope；\MCMTotalLength；\MCMAbstractLength。摘要不分小标题且不超过一页，结果数字与正文保持一致。}
...
\section{问题重述}
\MCMGuide
  {用自己的话把题面重组为可建模的输入、输出、限制和提交任务，使读者不回看题面也能明白研究对象。}
  {研究对象；已知量；各问输出；硬性约束；附件与结果文件要求；问题间依赖。}
  {客观转述，删去与求解无关的背景，使用“给定……求……”和“在……约束下确定……”；不照抄题面句子。}
  {\MCMRestatementLength；通常控制在约半页至一页，按分问列出任务，不复制大段题面。}
...
\MCMGuide
  ...
  {\MCMAnalysisLength；每问一段或一小节，承担从具体题面到抽象模型的桥梁，不以模型名称替代分析。}
...
\ifmcmguidance
  \begin{quote}\small
  \textbf{从网上模板迁移的边界：}旧 Word 模板中的 RMSE=3.26、MAPE=4.12\% 等数值只是排版示例，不能复制为本题结果；“误差小于 5\%”“普适性极强”等句子也只能在当前数据和检验实际支持时使用。每个检验先写对象、数据来源、误差定义和接受标准，再写通过或未通过及其对结论的影响。
  \end{quote}
\fi
...
\ifmcmguidance
  ...
  \noindent\textbf{使用时保留：}本参赛队在竞赛过程中使用了 AI 工具，主要用于【按实际情况填写简要用途】，详细使用情况见支撑材料。

  \begin{mcmguidebox}
    \small
    {\color{mcmguidehead}\bfseries 2026 年 AI 支撑材料填写要点}
    \MCMGuideField{必须提交}{使用 AI 的参赛作品须在支撑材料中提供名为\texttt{AI 工具使用详情.pdf}的文件；本目录提供同名 LaTeX 模板，可按实际记录填写后编译。}
    \MCMGuideField{四项内容}{工具名称及版本/型号；具体使用目的和环节；主要提示方式与典型交互；AI 输出的采纳、人工修改和核验情况。}
    \MCMGuideField{核验边界}{核心建模与分析由本队独立或主导完成；语言润色可以概括说明，公式、程序、数据、结果和结论等非语言润色内容须写清人工审查方式。}
    \MCMGuideField{记录方式}{典型交互建议保留 2--3 次，完整记录提示词、AI 回复核心内容、本队处理方式和具体核验动作；不得粘贴 API Key、账号密码或其他敏感信息。}
    \MCMGuideField{与旧网模板的差异}{网上模板中“在正文逐处标注 AI 生成内容、在参考文献列出 AI 工具”的要求不属于本次组委会 2026 年试行规定；本模板以组委会文件为准，除非当届赛区另有书面要求，不额外添加该项。}
    \MCMGuideField{匿名要求}{AI 使用详情及支撑材料同样不得出现队员、学校或赛区身份信息；声明必须与实际使用情况一致，不得把未经人工审查的 AI 输出作为核心建模或分析成果。}
  \end{mcmguidebox}
\else
  \ifdefined\MCMUsesAI
    本参赛队在竞赛过程中使用了 AI 工具，主要用于\MCMUseDescription，详细使用情况见支撑材料。
  \else
...
\MCMGuide
  {让评阅者能够从附录和支撑材料定位全部输入、代码、结果与 AI 使用记录。}
  {文件名及相对路径；文件类型；用途；输入/输出；运行入口；软件及版本；随机种子；与正文图表或结论的对应关系；\texttt{AI 工具使用详情.pdf}。}
  {只写实际存在的文件和实际执行过的命令，不把计划文件写成已完成结果。}
  {文件列表宜用紧凑表格；代码和大表格放后续附录，不在这里堆全文。}
  {本节可放文件树或索引表；不放包含身份信息的截图。}
  {支撑材料压缩包、论文附录和正文中的文件名完全一致，第三方能按入口复现关键结果。}
  {遗漏可运行源程序；代码依赖、版本或入口不明；把赛题原始数据误写成自主查阅数据；\texttt{AI 工具使用详情.pdf}未放入支撑材料；使用绝对路径或包含队员/学校信息。}
\ifmcmguidance
  \begin{table}[H]
    \centering
    \small
    \caption{支撑材料文件清单（示例结构，提交前替换为真实文件）}
    \label{tab:supporting-materials-file-list}
    \begin{tabular}{p{3.15cm}p{1.35cm}p{3.05cm}p{2.7cm}p{3.0cm}}
      \toprule
      相对路径/文件名 & 类型 & 用途 & 输入与输出 & 正文对应位置 \\
      \midrule
      \texttt{AI 工具使用详情.pdf} & PDF & AI 使用记录 & 无/使用记录 & AI 工具使用声明 \\
      \texttt{code/main.py} & PY & 主求解程序 & 数据/结果文件 & 图表或结果编号 \\
      \texttt{results/result.csv} & CSV & 结果数据 & 程序输出 & 表/图编号 \\
      \bottomrule
    \end{tabular}
  \end{table}
\fi
...
\MCMGuide
  {按组委会格式规范在附录中提供完整、可运行的源程序和必要支撑材料。}
  {程序语言及版本；入口文件；依赖包；运行命令；参数与随机种子；Excel/SPSS 等交互命令；数据和结果文件；较大篇幅中间图表；运行失败或无程序的说明。}
  {说明性文字短而实，代码保持原样或以可复制的代码环境呈现，不用代码截图替代源程序。}
  {代码按模块分节；大表和日志放支撑材料并在清单中索引；附录页数不限但不重复主文。}
  {正文引用的补充图表、代码模块和文件名应能在本附录或支撑材料中互相定位。}
  {第三方可按清单在干净环境运行入口并复核正文关键结果；若确实没有程序，明确写“本论文没有用到程序”。}
  {只给伪代码；漏掉 Excel/SPSS 命令；结果与正文不是同一版本；代码无法运行；把一次预期输出写成实际结果；附录或支撑材料出现身份信息。}
% 提交正文在此粘贴完整源程序，或按模块使用 listings/inputminted；没有程序时保留规定的明确说明。
