...
SOLVE_CUES = re.compile(
    r"(?:求解|计算|迭代|积分|离散|步长|网格|扫描|搜索|优化|训练|拟合|标定|"
    r"仿真|模拟|初始化|初值|终止条件|收敛|代入|调用|运行|RK4|FDM|FEM)"
)
STRONG_RESULT_CUES = re.compile(
    r"(?:结果|表\s*[0-9一二三四五六七八九十]+|图\s*[0-9一二三四五六七八九十]+|"
    r"高于|低于|相比|增加|减少|上升|下降|达到|波动|表明|说明|意味着)"
)
VALIDATION_CUES = re.compile(
    r"(?:敏感性|稳健性|鲁棒性|误差检验|离散误差|残差|交叉验证|独立复算|步长减半|"
    r"参数扰动|局部扰动|回代检验|收敛检验|边界检验|稳定率)"
)
INTERPRETATION_CUES = re.compile(
    r"(?:表明|说明|意味着|据此|只保留|方向性判断|结论|不能作为|不宜作为|"
    r"难以支持|不足以说明|可解释为)"
)
MODEL_ACTION_CUES = re.compile(
    r"(?:状态量|决策变量|目标函数|约束条件|边界条件|状态方程|控制方程|"
    r"转移关系|递推关系|Jacobian|雅可比|承载力|损失项|控制量|变量映射)"
)
...
HIGH_SHARE_ACTION = {
    "background": "检查是否题面复述或通用背景过长",
    "model": "检查是否重复定义、算法百科或公式后再释义过长",
    "solve": "检查是否把运行日志和重复步骤写入正文",
    "result": "检查是否结果堆叠、重复报数或把求解过程误归到结果",
    "validation": "检查是否重复检验同一结论或堆叠无解释的敏感性表",
    "evaluation": "检查是否通用优缺点、边界和改进建议反复出现",
}
LOW_SHARE_ACTION = {
    "background": "检查是否缺少任务口径、数据入口或必要假设",
    "model": "检查是否缺少变量关系、约束来源或公式解释",
    "solve": "检查是否缺少实际算法步骤、参数设置或求解接口",
    "result": "检查是否缺少关键数值、图表读数及其对象解释",
    "validation": "检查是否缺少误差、边界、灵敏度或独立复算",
    "evaluation": "检查是否缺少模型适用范围、局限或可执行改进",
}
...
def headings(text: str) -> list[Heading]:
    ...
    for index, item in enumerate(found):
        ...
        for candidate in found[index + 1:]:
            if rank[candidate.level] <= rank[item.level]:
                end = candidate.start
                break
...
def paragraph_category(titles: list[str], value: str) -> str:
    """Resolve composite headings with local prose instead of title order alone."""
    joined = " ".join(titles)
    local_title = titles[-1] if titles else ""
    composite_model_solve = (
        "\u6a21\u578b" in local_title and "\u6c42\u89e3" in local_title
    )
    local_base = category_for([local_title]) if local_title else "other"
    if (
        local_base in {"background", "model", "validation", "evaluation"}
        and not composite_model_solve
    ):
        return local_base
    if VALIDATION_CUES.search(value):
        return "validation"
    composite_solve_result = (
        any(token in joined for token in ("求解", "计算", "标定", "仿真", "优化"))
        and any(token in joined for token in ("结果", "对照", "比较"))
    )
    if composite_solve_result:
        solve_hits = list(SOLVE_CUES.finditer(value))
        result_hits = list(STRONG_RESULT_CUES.finditer(value))
        has_solve = bool(solve_hits)
        has_result = bool(result_hits) or bool(INTERPRETATION_CUES.search(value))
        if has_solve and not has_result:
            return "solve"
        if has_solve and has_result:
            if INTERPRETATION_CUES.search(value):
                return "result"
            # Procedure-first paragraphs remain solver prose when the local
            # computational evidence is at least as substantial as the result
            # wording. A result paragraph may mention one calculation in
            # passing, so result remains the conservative fallback.
            if len(solve_hits) >= len(result_hits) and solve_hits[0].start() < result_hits[0].start():
                return "solve"
            return "result"
    base = category_for(titles)
    if composite_model_solve and SOLVE_CUES.search(value):
        return "solve"
    return base


def action_distribution(
    text: str, spans: list[tuple[int, int]], headings_list: list[Heading]
) -> tuple[
    list[str],
    dict[str, dict[str, float | int]],
    dict[str, dict[str, float | int]],
]:
    """Measure local rhetorical actions without imposing a required order."""
    records = [
        (position, value)
        for start, end in spans
        for position, value in paragraphs(text, start, end)
    ]
    buckets: dict[str, list[str]] = {
        key: [] for key in (*SECTION_CATEGORIES.keys(), "other")
    }
    for position, value in records:
        buckets[paragraph_category(active_titles(headings_list, position), value)].append(value)
    total_han = sum(han_chars(value) for _, value in records) or 1
    distribution: dict[str, dict[str, float | int]] = {}
    for key, items in buckets.items():
        local_han = sum(han_chars(value) for value in items)
        distribution[key] = {
            "paragraphs": len(items),
            "han_chars": local_han,
            "share": round(local_han / total_han, 4),
        }
    evidence_buckets: dict[str, list[str]] = {
        key: [] for key in (*SECTION_CATEGORIES.keys(), "other")
    }
    for position, value in records:
        titles = active_titles(headings_list, position)
        title_category = category_for(titles)
        matched = False
        if title_category == "background":
            evidence_buckets["background"].append(value)
            matched = True
        if title_category == "model" or MODEL_ACTION_CUES.search(value):
            evidence_buckets["model"].append(value)
            matched = True
        if title_category == "solve" or SOLVE_CUES.search(value):
            evidence_buckets["solve"].append(value)
            matched = True
        if title_category == "result" or STRONG_RESULT_CUES.search(value) or INTERPRETATION_CUES.search(value):
            evidence_buckets["result"].append(value)
            matched = True
        if title_category == "validation" or VALIDATION_CUES.search(value):
            evidence_buckets["validation"].append(value)
            matched = True
        if title_category == "evaluation":
            evidence_buckets["evaluation"].append(value)
            matched = True
        if not matched:
            evidence_buckets["other"].append(value)
    evidence: dict[str, dict[str, float | int]] = {}
    for key, items in evidence_buckets.items():
        local_han = sum(han_chars(value) for value in items)
        evidence[key] = {
            "paragraphs": len(items),
            "han_chars": local_han,
            "coverage_share": round(local_han / total_han, 4),
        }
    return [value for _, value in records], distribution, evidence


QUESTION_TITLE = re.compile(
    r"^(?:问题\s*(?P<plain>[0-9一二三四五六七八九十百两]+)|"
    r"第\s*(?P<ordinal>[0-9一二三四五六七八九十百两]+)\s*问)"
    r"(?:\s*的)?(?:\s*(?:模型|建模|建立|求解)|\s*$)"
)
...
def question_number(title: str) -> int | None:
    match = QUESTION_TITLE.search(re.sub(r"\s+", "", title))
    if not match:
        return None
    return chinese_number(match.group("plain") or match.group("ordinal"))


def aux_section_pages(aux_text: str) -> list[int]:
    """Read top-level section start pages from ordinary LaTeX AUX writes."""
    pages: list[int] = []
    for line in aux_text.splitlines():
        if r"\contentsline {section}" not in line:
            continue
        match = re.search(r"\}\{(?P<page>\d+)\}\{(?:section|appendix)\.", line)
        if match:
...
def paragraphs(text: str, start: int, end: int) -> list[tuple[int, str]]:
    ...
    for match in chunk_pattern.finditer(prose):
        ...
        # Headings already travel separately through active_titles(). Keeping
        # their words in the paragraph would make a heading such as
        # "model establishment and solution" falsely mark every local
        # paragraph as solver prose and inflate lexical/result counts.
        prose_chunk = heading_pattern.sub(" ", chunk)
        value = re.sub(r"\s+", " ", visible_prose(prose_chunk)).strip()
...
def corpus_hints(metrics: dict, stats_path: Path | None, problem_type: str | None) -> list[str]:
    ...
    for category, names in CORPUS_GROUPS.items():
        ...
        if share > q3 * 1.35:
            hints.append(
                f"{category} 篇幅占比 {share:.1%} 高于语料四分位上沿 {q3:.1%}，"
                f"{HIGH_SHARE_ACTION[category]}"
            )
        elif share < q1 * 0.65:
            evidence_share = float(
                metrics.get("action_evidence", {}).get(category, {}).get("coverage_share", 0)
            )
            if evidence_share >= q1 * 0.65:
                hints.append(
                    f"{category} 作为段落主功能的篇幅占比 {share:.1%} 低于语料四分位下沿 {q1:.1%}，"
                    f"但相关动作证据覆盖 {evidence_share:.1%}；检查是否长期与其他功能混写，"
                    "使步骤、结果和解释的停点不清"
                )
            else:
                hints.append(
                    f"{category} 篇幅占比 {share:.1%} 低于语料四分位下沿 {q1:.1%}，"
                    f"{LOW_SHARE_ACTION[category]}"
                )
    return hints
...
def question_metrics(text: str, aux: str, question: dict, headings_list: list[Heading]) -> dict:
    ...
    if not spans:
        return {
            ...
            "action_distribution": {
                key: {"paragraphs": 0, "han_chars": 0, "share": 0.0}
                for key in (*SECTION_CATEGORIES.keys(), "other")
            },
            "action_evidence": {
                key: {"paragraphs": 0, "han_chars": 0, "coverage_share": 0.0}
                for key in (*SECTION_CATEGORIES.keys(), "other")
            },
            "status": "unlocated",
        }
    blocks = [text[start:end] for start, end in spans]
    values, local_actions, local_evidence = action_distribution(text, spans, headings_list)
    return {
        "id": question_id,
        "pages": None if start_page is None or end_page is None else end_page - start_page + 1,
        ...
        "action_distribution": local_actions,
        "action_evidence": local_evidence,
        "status": "located",
    }


def inferred_question_metrics(text: str, aux: str, headings_list: list[Heading]) -> list[dict]:
    """Measure conventional Chinese question sections when no coverage file exists."""
    sections = [item for item in headings_list if item.level == "section"]
    section_pages = aux_section_pages(aux)
    page_by_start = {
        section.start: section_pages[index]
        for index, section in enumerate(sections)
        if index < len(section_pages)
    }
    output: list[dict] = []
    for index, section in enumerate(sections):
        number = question_number(section.title)
        if number is None:
            continue
        block = text[section.start:section.end]
        values, local_actions, local_evidence = action_distribution(
            text, [(section.start, section.end)], headings_list
        )
        start_page = page_by_start.get(section.start)
        next_page = None
        if index + 1 < len(sections):
            next_page = page_by_start.get(sections[index + 1].start)
        pages = None
        if start_page is not None:
            pages = max(1, next_page - start_page) if next_page is not None else None
        output.append({
            "id": f"Q{number}",
            "title": section.title,
            "pages": pages,
            "paragraphs": len(values),
            "han_chars": sum(han_chars(value) for value in values),
            "formulas": tex_count(block, rf"\\begin\{{{FORMULA_ENVS}\}}|\\\["),
            "figures": tex_count(block, rf"\\begin\{{{FIGURE_ENVS}\}}"),
            "tables": tex_count(block, rf"\\begin\{{{TABLE_FLOAT_ENVS}\}}"),
            "tabular_blocks": tex_count(block, rf"\\begin\{{{TABULAR_ENVS}\}}"),
            "result_explanations": sum(bool(RESULT_CUES.search(value)) for value in values),
            "action_distribution": local_actions,
            "action_evidence": local_evidence,
            "status": "inferred",
...
def audit(
    ...
    if start_marker >= 0:
        start_method = "explicit-label"
    if start_marker < 0:
        standard_start = raw.find(r"\begin{document}")
        if standard_start >= 0:
            start_marker = standard_start
            start_method = "document-environment"
    if end_marker >= 0:
        end_method = "explicit-label"
    if end_marker < 0:
        standard_end = raw.rfind(r"\end{document}")
        if standard_end >= 0:
            end_marker = standard_end
            end_method = "document-environment"
    body_start = start_marker if start_marker >= 0 else 0
    body_end = end_marker if end_marker >= 0 else len(raw)
    if end_marker < 0:
        appendix = raw.find(r"\appendix", body_start)
        if appendix >= 0:
            body_end = min(body_end, appendix)
    ...
    for position, value in paragraphs(raw, body_start, body_end):
        titles = active_titles(heading_list, position)
        category_values[paragraph_category(titles, value)].append(value)
    categories = {
        key: {
            "paragraphs": len(items),
    ...
    _, _, body_action_evidence = action_distribution(
        raw, [(body_start, body_end)], body_headings
    )
    metrics = {
        "paragraphs": len(values),
        "han_chars": sum(han_chars(value) for value in values),
        ...
        "action_evidence": body_action_evidence,
    }
    ...
    if coverage_path and coverage_path.is_file():
        ...
        questions = inferred_question_metrics(raw, aux, body_headings)
        if not questions:
    ...
    return {
        ...
        "source": {
            ...
            "sha256": hashlib.sha256(main_tex.resolve().read_bytes()).hexdigest(),
        },
        "problem_type": problem_type,
        "body_boundary": {
            "method": (
                "explicit-labels" if start_method == end_method == "explicit-label"
                else "document-environment" if start_method == end_method == "document-environment"
                else "mixed" if start_method and end_method else "fallback"
            ),
            "start_found": start_marker >= 0,
            "end_found": end_marker >= 0,
        },
        "body": metrics,
        "questions": questions,
...
def main() -> int:
    ...
    if args.format == "json":
        ...
        for question in report["questions"]:
            primary = ",".join(
                f"{key}:{question['action_distribution'][key]['paragraphs']}"
                for key in ("background", "model", "solve", "result", "validation", "evaluation")
            )
            evidence = ",".join(
                f"{key}:{question['action_evidence'][key]['paragraphs']}"
                for key in ("model", "solve", "result", "validation")
            )
            print(
                ...
                f"primary={primary} evidence={evidence}"
            )
