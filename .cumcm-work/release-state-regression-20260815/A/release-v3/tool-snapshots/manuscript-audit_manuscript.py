...
SECTION_RULES = {
    ...
    "results": ("结果分析", [r"结果分析", r"结果与分析", r"求解结果"]),
    "validation": ("模型检验", [r"模型检验", r"模型验证", r"误差分析", r"有效性检验"]),
    "robustness": ("灵敏度或稳健性分析", [r"灵敏度", r"敏感性", r"稳健性", r"参数扰动"]),
    "evaluation": ("模型评价与改进", [r"模型评价", r"优缺点", r"模型改进", r"改进方案"]),
    "references": ("参考文献", [r"\\begin\s*\{thebibliography\}", r"\\bibliography\s*\{"]),
...
HEADING_LEVEL = {"section": 1, "subsection": 2, "subsubsection": 3}
...
def required_sections(text: str) -> list[Finding]:
    ...
    expected = list(SECTION_RULES)
    observed = [key for key, _ in sorted(positions, key=lambda item: item[1])]
...
def cross_references(text: str) -> list[Finding]:
    findings = []
    ...
    if citations and not re.search(r"\\bibliography\s*\{", text):
        bibitems = set(re.findall(r"\\bibitem(?:\[[^]]*\])?\s*\{([^}]+)\}", text))
        for citation in sorted(citations - bibitems):
            findings.append(Finding("UNDEFINED_CITATION", "error", f"引用键“{citation}”没有对应 bibitem。"))
    return findings
...
def semantic_signals(text: str, problem_type: str) -> list[Finding]:
    findings = []
    result = section_body(text, r"结果(?:分析|与分析)")
    if not result:
...
def audit(path: Path, problem_type: str) -> dict:
    text = read_tex_tree(path)
    findings = (required_sections(text) + placeholders(text) + floats(text)
                + cross_references(text) + semantic_signals(text, problem_type)
...
def main() -> int:
    ...
    try:
        result = audit(args.main_tex, args.problem_type)
