...
\begin{enumerate}
\item Section B 不能只用“单题均值”理解，实际判断还要看单题分布、整套容量、边缘带、条件联动和反向排除。
...
\item 信息量较大的往往是 \CodeInline{0-20\%} 和 \CodeInline{80-100\%} 这类边缘带。题目落入边缘带后，位置本身就可作为条件信号。
\item 对考生有用的不是“猜某题均值落点”，而是先用粗分布确定大区，再用容量约束检查是否挤得过满，最后用条件规则缩小搜索窗；if-else 只负责固定这个顺序。
\end{enumerate}
...
为便于查找，本文作为 \CodeInline{\_analysis\_output} 下 Section B 分析的\textbf{单一主入口}。以下几份结论型 Markdown 已并入：
...
这些以 \CodeInline{paragraph\_position\_pct} 为基础的位置表也来自同一批完整样本：
...
所以，段落位置、相关性、容量约束和条件联动均采用 \textbf{59 套完整样本}。
...
这 \CodeInline{3} 篇文章后来补入了 \CodeInline{section\_b\_texts} 标准链路：
...
不过，大多数字母的 \CodeInline{reverse\_entropy} 仍不低，字母层面只能支持两点：
...
\begin{itemize}
\item 扫描前段时，它比 \CodeInline{Q36} 更可能先露头
\end{itemize}
...
\begin{enumerate}
\item 先看单题粗分布，确定前、中、后哪个区域更值得先扫。
\item 接着核对整套容量，已经挤满的区域就把下一题移开。
\item 再看题间的无条件关系，避开把相邻题当成邻近题的直觉。
\item 真正落到某一道题时，再调用条件联动规则。
\end{enumerate}
...
做 Section B 时，10 题不是十个互不相干的小题；前面确认的位置会改变后面的搜索范围。实际操作可按下面的顺序走：
...
**单题粗分布先给出大区，容量据此校正，再确认锚点。出现边缘带时立即缩窗：\CodeInline{Q36} 用来定区，\CodeInline{Q44} 用来传递条件，\CodeInline{Q38} 用来反推；前段查 \CodeInline{Q37/Q39}，后段查 \CodeInline{Q41/Q45}，不要按 \CodeInline{36 -> 37 -> ... -> 45} 顺序硬扫。**
...
\begin{enumerate}
\item \textbf{第一层：先找低成本锚点。} 在 \CodeInline{Q37 / Q39 / Q41 / Q45} 中选题干容易定位的一题；\CodeInline{Q36} 用来定区；\CodeInline{Q38 / Q42 / Q43} 暂不裸做。
