...
\textbf{本文件的依据与填写口径。}披露方式以国规《全国大学生数学建模竞赛人工智能工具使用规定
（2026 年试行）》为准；本文件结构按其第 4 条的四项要求编排，四个小节与四项一一对应，
版式参考指导教师建议的样例。本文件只记录竞赛期间真实发生的使用情况：没用过的工具不列，
没发生的交互不编。文件名须为 \texttt{AI 工具使用详情.pdf}，随支撑材料提交。本文件、论文附录与
支撑材料压缩包的任何位置（含文件夹名、文件名、文档属性）都不得出现队员姓名、学校或赛区信息，
也不得写入账号、API Key 等敏感信息。
}}
...
\vspace{1.2em}
\noindent\fbox{\parbox{0.965\linewidth}{\small
\textbf{正文侧还须完成一件事，本文件不能替代。}

按国规《人工智能工具使用规定（2026 年试行）》第 3 条，在论文\textbf{参考文献之前}设
“AI 工具使用声明”，下列二者\textbf{择一}，不得同时保留：
...
国规第 4 条只要求上述声明加本 PDF，\textbf{没有}要求在正文逐处标注 AI 生成内容，
也\textbf{没有}要求把 AI 工具另列为参考文献。国规第 6 条写明“自 2026 年 9 月 1 日起试行，
此前相关规定与本规定不一致的，以本规定为准”，故披露方式一律以国规为准。若当届组委会
另发书面通知，以通知为准。
}}

\vspace{0.8em}
\noindent\fbox{\parbox{0.965\linewidth}{\small
\textbf{提交环节的硬约束：MD5。}本文件与参赛论文都要参与 MD5 登记，时序错了会直接失去资格。

\begin{itemize}
  \item 在规定时限内通过客户端提交“参赛论文”与“支撑材料”对应文件的 MD5 码，
        可多次上传，以最后一次为准，过时无效。
  \item \textbf{只要对文件执行过打开保存（含自动保存），MD5 就会改变}，必须重新生成
        并在截止前重新提交。上传 MD5 前先关闭文件。
  \item 每次上传成功后备份对应文件，\textbf{备份文件不得点击打开}。
  \item 建议提前至少 1 小时完成上传；网络异常时改用手机热点。
  \item 之后在规定时段上传与已提交 MD5 对应的电子文档。
\end{itemize}

对本文件的直接含义：\textbf{生成本 PDF 并登记 MD5 之后，不要再打开它或它的源文件}。
必须修改时按“改完—关闭—重新生成 MD5—重新提交”执行，并留出重传的时间余量。
具体日期与客户端要求见当届赛区通知。
}}
